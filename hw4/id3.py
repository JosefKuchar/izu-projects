...
objects = """1 Q velky dlouha cerny ano
    2 N maly bez bily ne
    3 V stredni bez hnedy ne
    4 V stredni dlouha cerny ne
    5 N maly kratka hnedy ne
    6 N stredni dlouha hnedy ne
    7 V stredni kratka cerny ne
    8 N velky bez cerny ne
    9 V velky kratka cerny ne
    10 Q maly bez cerny ano
    11 V stredni bez cerny ano
    12 N maly bez bily ano
    13 V velky kratka bily ano
    14 Q stredni kratka bily ne
    15 N velky kratka hnedy ano
"""
...
def recursion(string, obj, att, nameIndex):
    ...
    for j in range(1, num_of_att+1):
        ...
        Gain.append(EntS - (clbSum[0]/claSum)*subEntAtt[0] - (clbSum[1] /
                    claSum)*subEntAtt[1] - (clbSum[2]/claSum)*subEntAtt[2])
    ...
    string = string[:-1] + "}"
    buffer += "uzel" + \
        str(nameIndex) + " [" + record + "label=\"" + \
        att[max_index*2] + "|" + string + "\"]\n"
    ...
    for i in att[max_index*2+1]:
        ...
        string = string[:-1] + "}"
        if len(string) > 2:
            buffer += "uzel" + str(nameIndex) + " -> uzel" + str(
                (nameIndex+j)*10) + " [label=\"" + i + " " + string + "\"]\n"
            tempAtt.clear()
            for item in att:
                tempAtt.append(item)
